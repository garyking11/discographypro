...
$(document).ready(function() {
	...
	$("#edit-image").on('click', function(e) {									
		...
		//var action = $('#media-action').val(); 
		var action = $('form#album-form').attr('action');
		var item1 = $('#allImage').val();
		var item2 = $("#allImageCaption").val();
	/*	var item3 = $("#media_type").val();
		var item4 = $("#").val();
		var item5 = $("#").val();
		var item6 = $("#").val();
		var item7 = $("#").val();
	...
	$.ajax({	
		...
		success: function(data) {
			//alert(item1);
			//$('#image-scripts1').html("<?php include 'http://dev.discographypro.com/dev/site/config/database.php'; $db = new \Database\Database; ?>")	
		  ...
		  $("#edit-image").removeClass('btn btn-warning');
			...
			$("#image-scripts3").html(aid + ' image scripts 3');
...
// Add image to slideshow
	...
	// Show the thumbnails of selected slideshow images
			...
			+ 'src="' +  '../../site/assets/images/' + component + '/' +  id + '/' + selectImage + '" />'
	...
	// Add a caption box to the image
			...
			+  'value="' + selectImage + '" /></div>'//Site/Assets/Images/album/' +  id + '/		
